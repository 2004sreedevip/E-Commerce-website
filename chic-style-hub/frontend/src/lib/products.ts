  ...
  for (const [subcategory, items] of Object.entries(groups)) {
    for (const s of items) {
      out.push({
        ...
        price: s.price,
        mrp: s.mrp,
        image: `/images/Images/${category}/${LOCAL_IMAGES[category][imageIndex++ % LOCAL_IMAGES[category].length]}`,
        category,
        subcategory,
        fastDelivery: i % 2 === 0,
      });
...
// Generate a guaranteed-unique image per product using local images.
const uniqueImg = (key: string, cat: Product["category"]) =>
  `/images/Images/${cat}/${LOCAL_IMAGES[cat][Math.abs(key.split('').reduce((a,b)=>a+b.charCodeAt(0),0)) % LOCAL_IMAGES[cat].length]}`;
...
function expand(base: Product[], prefix: string, cat: Product["category"]): Product[] {
  ...
  const enrich = (p: Product, idx: number): Product => {
    let image = p.image;
    if (seenImages.has(image)) image = uniqueImg(`${prefix}-${p.id}-${idx}`, cat);
      ...
      enrich(
        {
          ...
          price,
          mrp,
          image: uniqueImg(`${prefix}-${newId}-${variant}`, cat),
